...
FServerSideRewindResult ULagCompensationComponent::ServerSideRewind(class ABlastCharacter* HitCharacter, const FVector_NetQuantize& TraceStart, const FVector_NetQuantize& HitLocation, float HitTime)
{
	FFramePackage FramePackage = GetFrameToCheck(HitCharacter, HitTime);
	return ConfirmHit(FramePackage, HitCharacter, TraceStart, HitLocation);
}
...
FShotgunServerSideRewindResult ULagCompensationComponent::ShotgunConfirmHit(const TArray<FFramePackage>& FramePackages, const FVector_NetQuantize& TraceStart, const TArray<FVector_NetQuantize>& HitLocations)
{
	...
	{
		FHitResult ConfirmHitResult;
		FVector TraceEnd = TraceStart + (HitLocation - TraceStart) * 1.23f;
		World->LineTraceSingleByChannel(ConfirmHitResult, TraceStart, TraceEnd, ECC_HitBox);

		if (World)
		{
			if (ABlastCharacter* BlastCharacter = Cast<ABlastCharacter>(ConfirmHitResult.GetActor()))
			{
	...
	// enable collision for all boxes, then disable for head boxAdd commentMore actions
	for (auto& Frame : FramePackages)
	{
...
void ULagCompensationComponent::SaveFramePackage()
{
	...
	else
	{
		...
		//ShowFramePackage(ThisFrame, FColor::Red);Add commentMore actions
	}
}

//给Package里填满信息(Time和当前时间下各Box的Transform)
void ULagCompensationComponent::SaveFramePackage(FFramePackage& Package)
{
	...
	if (Character)
	{
		...
		for (auto& BoxPair: Character->HitCollisionBoxes)
		{
			...
			Package.HitBoxInfo.Emplace(BoxPair.Key, BoxInformation);
		}
	}
}
